...
class Clients extends ParentController
{
    ...
    */
    ...
    //Client profile Page For Client Portal
    public function client_settings(){
        parent::get_client_nofif_data();
        $check_session = DB::table('clients')->where('client_login_session', Cookie::get('client_session'))->first();
        if(!$check_session){
            return redirect('/cout');
        }else{
        $notification_codes = DB::table('notifications_code_client')->get();
        return view('clients.client_setting', ['name' => $check_session, 'notifications_codes' => $notification_codes, 'notifications_counts' => $this->notif_counts_client, 'notif_data' => $this->notif_data_client, 'all_notif' => $this->clients_all_notifications]);
        }
        
    }

    public function change_pass_client(Request $request){
        
        $client = DB::table('clients')->where('client_login_session', Cookie::get('client_session'))->first();


        if (Hash::check($request->current_pass, $client->password)) {
            //$employee->password = bcrypt($request->confirm_password);
            $update = DB::table('clients')->where('client_login_session', Cookie::get('client_session'))->update(['password' => bcrypt($request->new_pass)]);
            if($update){
                echo json_encode('success');
            }else{
                echo json_encode('failed');
