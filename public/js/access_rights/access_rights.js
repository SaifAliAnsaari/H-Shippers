...
function fetchRightsAgainstEmployee(){
    var segments = location.href.split('/');
    var id = segments[4];
    $.ajax({
        type: 'GET',
        url: '/check_access_rights',
        data: {
            _token: '{!! csrf_token() !!}',
            id: id
        },
        success: function(response) {
            //console.log(response);
            if(JSON.parse(response) == "no_user"){
                $('#access_rights_div').show();
                $('#dataSidebarLoader').hide();
            }else {
                //Yaha response ma say right nikal k show krnay hain
                $('#access_rights_div').show();
                $('#dataSidebarLoader').hide();
                alert('here');
            }
